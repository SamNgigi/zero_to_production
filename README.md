
# Zero to Production

Covering Luca Palmieri's book to understand backend web services development in Rust. 

Here we
- Update the original Actix-web implementation for 2026
- Re-implement in Axum.

⚠️ Hand-written code for the purposes of learning.

> **NOTE**:
> Uses Rust based Jupyter Notebook(via [evcxr](https://www.freecodecamp.org/news/how-to-run-rust-on-jupyter-notebooks/)) for notes instead of raw Markdown to explore some of the crates
> used in the project.
> 
> If we there's no need to explore a chapter via a jupyter notebook, goal is to convert the notebooks to markdown files via the command
> ```bash
> jupyter nbconvert --to markdown notebook_name.ipynb
> ```

