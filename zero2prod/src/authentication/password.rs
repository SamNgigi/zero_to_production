...
#[tracing::instrument(name = "Update Password", skip(db_pool, user_id, new_password))]
pub async fn update_password(
    ...
    let password_hash = spawn_blocking_with_tracing(move || compute_password_hash(new_password))
        .await
        .context("Failed to compute password hash for new password.")??;
    ...
    .execute(db_pool)
    .await
    .context("Failed to execute SQL query to update user password.")?;
...
fn compute_password_hash(password: SecretString) -> Result<SecretString, anyhow::Error> {
    ...
    let password_hash = Argon2::new(
        Algorithm::Argon2id,
        Version::V0x13,
        Params::new(19000, 2, 1, None).expect("Failed to build Argon2 params"),
    )
