...
async fn worker_loop(db_pool: PgPool, email_client: EmailClient) -> Result<(), anyhow::Error> {
    loop {
        match try_processing_task(&db_pool, &email_client).await {
            ...
            Err(_) => {
                // NOTE: Pause for 1 sec in case of transient errors.
                // TODO: We can make robust by emporing `worker_loop` to differential
                // between transient and non transient errors and handle them appropriately.
...
pub async fn try_processing_task(
    ...
    tracing::Span::current()
        ...
        .record("Subscriber Email", tracing::field::display(&email));
        ...
        Ok(email) => {
                ...
                .await
            {
                tracing::error!(
                    error.cause_chain = ?e,
                    error.message = %e,
                    "Failed to send newsletter issue to confirmed subscriber. \
                     Skipping."
                );
            }
        }
        Err(e) => {
            tracing::error!(
                ...
                "Skipping confirmed subscriber. \
                 Stored subcriber details are invalid."
            );
        }
    }
...
async fn dequeue_task(
    ...
    let row = sqlx::query!(
        r#"
            SELECT newsletter_issue_id, email
    ...
    if let Some(row) = row {
        Ok(Some((transaction, row.newsletter_issue_id, row.email)))
    } else {
        Ok(None)
    }
}
