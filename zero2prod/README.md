# Zero To Production in Axum

Re-implementation/porting of the Zero To Production in Axum from Actix-web

Just completed Chapter 5. Succesfull deployment.

```bash
curl -v https://zero2prod-axum-impl.fly.dev/health_check
curl -d "username=lei%20yin&email=lei_yin_loo%40gmail.com" https://zero2prod-axum-impl.fly.dev/subscriptions
```
or equivalent post request
```bash
curl --request POST \
    --data 'username=lei%20yin&email=lei_yin_loo%40gmail.com' \
    https://zero2prod-axum-impl.fly.dev/subscriptions \
    --verbose
```

