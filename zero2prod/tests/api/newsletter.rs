...
async fn transient_errors_do_not_cause_duplicate_deliveries_on_retry() {
    ...
    // NOTE: Act & Assert 1 -> First request - successfull delivery
    ...
    // NOTE: Act & Assert 2 -> Second request - failed delivery due to transient 500,
    Mock::given(path("/email"))
        .and(method("POST"))
    ...
    // NOTE: Act & Assert 3 -> Retry request - Should only retry the failed delivery.
    Mock::given(path("/email"))
        .and(method("POST"))
        .respond_with(ResponseTemplate::new(200))
        .expect(1) // NOTE: Expecting one retry request for failed delivery
